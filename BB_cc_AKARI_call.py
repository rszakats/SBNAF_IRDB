...
# RS - mod to be callable finction from python3, mod for MSX
import sys
'''
###################################################
if len(sys.argv) < 2:
...
lambda_ref=[4.0,7.0,9.0,11.0,15.0,18.0,24.0]
...
num_rows = np.zeros(7, dtype = np.int64)
l_filt  = np.zeros((7,3000)) - 1.0
r_filt  = np.zeros((7,3000))
re_filt = np.zeros((7,3000)) - 1.0
f_data  = np.zeros((7,3000)) - 1.0
...
def read_filters():
    ...
    filenames=['N4.dat','S7.dat','s9w.txt','S11.dat','l15.fad','l18w.txt','l24.fad']
    for K in range(7):
